...
log_name = 'dgl_cluster-ogbn-products-cpugpu'
...
model = ClusterGCN(graph.ndata['feat'].shape[1], 256, dataset.num_classes).cuda()

opt = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=0)

num_partitions = 2000
sampler = dgl.dataloading.ClusterGCNSampler(graph, num_partitions, cache_path='cache/cluster_gcn-products.pkl')
...
print('DGL, ogbn-products, ClusterGCN, cuda')
