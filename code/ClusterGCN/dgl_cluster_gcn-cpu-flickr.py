...
log_name = 'dgl_cluster-flickr-cpu'
...
model = ClusterGCN(graph.ndata['feat'].shape[1], 256, num_classes).to('cpu')
opt = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=0)

num_partitions = 2000
sampler = dgl.dataloading.ClusterGCNSampler(graph, num_partitions, cache_path='cache/cluster_gcn-flickr.pkl')
...
print('DGL, flickr, ClusterGCN, cpu')
