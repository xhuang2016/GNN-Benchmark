...
log_name = 'dgl_graphsage-ogbn-products-gpu'
...
model = GraphSAGE(graph.ndata['feat'].shape[1], 256, num_classes).to(device)
...
#         graph, torch.arange(graph.num_nodes()).to(device), sampler, device=device, batch_size=512, shuffle=True,
#         drop_last=False, num_workers=0, use_uva=False)


print('DGL, products, GraphSAGE, mini-batch, gpu')
