...
def train(model, feats, y_true, train_idx, optimizer):
    ...
    loss = loss_fcn(out, y_true[train_idx])
    loss.backward()
    optimizer.step()
...
parser = argparse.ArgumentParser(description='Flickr (GraphSAGE Full-Batch)')
...
log_name = 'dgl_graphsage-full-batch-flickr-' + device
...
print('DGL, flickr, GraphSAGE, full-batch, ' + device)
