...
log_name = 'dgl_graphsaint-ppi-cpu'
...
model = GraphSAINT(graph.ndata['feat'].shape[1], 256, num_classes).to('cpu')
...
dataloader = dgl.dataloading.DataLoader(graph, torch.arange(1), sampler, batch_size=1, num_workers=0, device='cpu', shuffle=True,
                                        drop_last=False, use_uva=False)


print('DGL, ppi, GraphSAINT, cpu')
